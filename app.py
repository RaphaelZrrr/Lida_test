# app.py
# ------------------------------------------------------------
# Streamlit app — "LLM → code → chart" (version light)
#
# Prérequis:
#   pip install streamlit pandas matplotlib seaborn requests numpy
#   ollama serve
#   (optionnel) ollama pull gpt-oss:20b  /  mistral:latest
#
# Lancer:
...
def pick_candidates_from_schema(schema: Dict[str, Dict[str, Any]], max_each: int = 25) -> Tuple[List[str], List[str], List[str]]:
    ...
    for col, meta in schema.items():
        if meta.get("missing_ratio", 0.0) > 0.95:
            continue
...
def exec_code_to_png(code: str, df: pd.DataFrame) -> Tuple[Optional[bytes], Optional[str]]:
    """
    Execute generated code and capture current matplotlib figure as PNG bytes.
...
st.title("LLM Viz — Light")
st.caption("Upload → prompt → code généré → graphique. Backend: Ollama compatible OpenAI.")
...
with st.sidebar:
    ...
    sample_rows_jsonl = st.number_input("JSONL sample rows", min_value=100, max_value=500000, value=50000, step=1000)
    sample_n_schema = st.number_input("Schema sample_n", min_value=100, max_value=20000, value=800, step=100)
    max_each_candidates = st.number_input("Candidates max_each", min_value=10, max_value=200, value=25, step=5)

    st.subheader("Affichage")
    show_raw = st.checkbox("Show raw model output", value=False)
    show_schema = st.checkbox("Show candidates", value=True)
...
with col_left:
    ...
    uploaded = st.file_uploader("CSV or JSONL", type=["csv", "jsonl", "log", "txt"])
...
with col_right:
    ...
    default_prompt = "Répartition des entités par ForceIdentifier"
    ...
    if run_btn and df is not None:
        ...
        if result.get("candidate_cols") and show_schema:
            with st.expander("Candidates (cat / num / time)", expanded=False):
    ...
    st.divider()
    st.caption("Tips: reste sur temperature=0 pour du code plus stable. Évite les prompts trop longs." )